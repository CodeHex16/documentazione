...
== Scopo del prodotto
Il software da realizzare consiste in un chatbot avanzato, basato su modelli linguistici (LLM), che un fornitore, ad
esempio di bevande o alimenti, può offrire ai propri, i quali possono ottenere in modo semplice e immediato informazioni
dettagliate sui prodotti o servizi disponibili, senza la necessità di contattare direttamente l'azienda.
...
- Admin non registrato;\
*Descrizione:*\
- Un admin non registrato cercando di registrarsi per interagire e usufruire dei servizi offerti dalla web app non ci è
...
- Il sistema ha ricevuto le credenziali per la registrazione ma queste non sono valide e pertanto viene inviato all’admin
  un messaggio d'errore in cui viene specificato che non può essere registrato;\
...
*Postcondizioni:*\
- Il sistema ha ricevuto le credenziali inviate dall’utente/admin, ma l’e-mail è già registrata nel sistema, quindi invia
  un messaggio d’errore all’utente;\
*Scenario Principale:*\
...
*Scenario Principale:*\
- L’utente/admin ha inserito l’indirizzo e-mail e l’username al fine di registrarsi;\
- Il sistema invia un messaggio d’errore indicando all’utente che l’username inserito è già in uso nel sistema e il
  tentativo di registrazione fallisce;\
...
- Il sistema ha ricevuto le credenziali inviate dall’utente/admin, e se sono valide permette l'accesso dell'utente/admin
  alla web-app e ai suoi servizi;\
*Scenario Principale:*\
...
- Utente/Admin registrato;\
*Descrizione:*\
- Un utente/admin registrato vuole mantenere l'accesso attivo al fine di non dover effettuare il login, inserendo le
...
*Postcondizioni:*\
- L’utente/admin ha inserito le credenziali (che devono essere valide), premuto il pulsante "Resta connesso" ed effettuato
  il login;\
*Scenario Principale:*\
- L’utente/admin inserisce le credenziali, preme il pulsante "Resta connesso" ed effetua il login;\
- Il sistema riceve la richiesta dell'utente/admin di accedere e se le credenziali sono corrette il login ha successo e il
  sistema mantiene in memoria le credenziali per un futuro login senza dover richiederle all'utente/admin;\
...
*Precondizioni:*\
- L'utente/admin deve essere in possesso dell'e-mail/username e della password temporanea inviatagli, tramite e-mail, alla
  registrazione;\
*Postcondizioni:*\
...
- Utente/Admin registrato;\
*Descrizione:*\
- Un utente/admin registrato cerca di effettuare il login per interagire e usufruire dei servizi offerti dalla web app non
...
*Precondizioni:*\
- L'utente/admin prima di effettuare il login ha premuto il pulsante "Passowrd dimenticata", oppure\
- L'utente/admin deve accedere alle impostazioni e alla sezione "Recupero password";\
- L'utente7admin deve possedere l'e-mail/username a cui è associato il suo account e inserirla nella sezione apposita così
...
- Utente/Admin registrato;\
*Descrizione:*\
- L'utente/admin ha ricevuto la risposta dal chatbot e vuole dare un feedback su tale risposta tramite l'opzione "Pollice
  su/giù";\
*Precondizioni:*\
- L'utente/admin deve aver effettuato il login;\
- L'utente/admin deve aver inviato un messaggio al chatbot;\
- L'utente/admin deve aver ricevuto una risposta dal chatbot;\
- L'utente/admin deve premere il "Pollice in su" se pensa che la risposta sia stata coerente con la domanda, altrimenti

...
= Scopo dell'incontro
L'incontro, svolto subito dopo la revisione #gloss[RTB] con il prof. Cardin, aveva lo scopo di presentare il #gloss[PoC] realizzato dal gruppo all'azienda Ergon e di chiarire alcuni aspetti sull'applicazione.
...
+ _Va bene avere una pagina singola di login sia per utente che #gloss[fornitore], con l'applicazione che controlla subito dopo il login se l'utente appena autenticato è un fornitore?_\
  Sì, la soluzione è corretta e ragionevole.

+ _Nel caso in cui le #gloss[FAQ], ovvero le coppie domanda-risposta preimpostate dal fornitore, siano troppo lunghe e occupino troppo spazio nell'interfaccia, può essere mostrata una versione abbreviata generata dal modello #gloss[LLM] o inserita direttamente dal fornitore?_\
  Sì, è possibile. Entrambe le opzioni vanno bene, è a vostra discrezione quale usare.

= Altri problemi
Durante l'incontro è stato evidenziato dal gruppo che il #gloss[chatbot], quando dovrebbe rispondere usando diversi documenti come contesto, fatica a dare una risposta completa. Una causa di questo problema è la struttura del dataset di test. L'azienda si impegnerà a fornire un dataset con un indice per aiutare a risolvere il problema.

...
+ _Possiamo ipotizzare la chat come schermata principale?_\
  \Sì, subito dopo l'autenticazione la schermata principale sarà la chat tra utente e IA.
   La grafica può essere semplice ma vengono comunque apprezzate funzionalità aggiuntive come ad esempio la scelta tra dark/light mode.
...
+ _E' necessario usare un logo in particolare?_\
  No, si può il logo del gruppo anche perché in fase di utilizzo ogni istanza del'app sarà personalizzata in base al fornitore; nonostante questo potrebbe essere interessante dare la possibilità al fornitore di scegliere logo e impostare una palette colori per il suo chatbot.
...
+ _E' necessario suggerire all'utente le domande da proporre?_\
  \Non è necessario, inoltre l'ideale sarebbe che l'LLM riuscisse a restituire delle risposte predefinite (dal configuratore del chatbot) anche se la domanda posta è solo simile alla domanda predefinita associata a quella risposta. 

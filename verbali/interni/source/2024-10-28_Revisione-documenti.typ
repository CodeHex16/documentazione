...
Abbiamo introdotto l'uso delle GitHub Issues come sistema di tracciamento delle attività da assegnare ai membri del gruppo. Questo strumento ci permette di gestire e monitorare le attività in modo efficace, favorendo una chiara suddivisione dei compiti.

Oltre ad aver già creato alcune issue iniziali, abbiamo impostato una prima milestone relativa alla fase di candidatura, in modo da avere una panoramica delle attività necessarie per il completamento di questa fase. Inoltre, abbiamo configurato una GitHub board per visualizzare lo stato di avanzamento delle issue, suddividendole in tre colonne: To Do, In Progress, e Done. Questo sistema ci consente di monitorare facilmente il progresso di ogni attività e di aggiornare lo stato di ciascun task in tempo reale.